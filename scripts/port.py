...
class Port(ast.NodeTransformer):
    ...
    def visit_Assign(self, node):
        ...
        for target in node.targets:
            if isinstance(target, ast.Name) and target.id.startswith("log"):
                return None
            if isinstance(target, ast.Name) and target.id == "recipe":
                node.value.func.id = "Recipe"
            if isinstance(target, ast.Attribute) and target.value.id.startswith("log"):
                if target.attr == "brew_date":
                    self.brew_date = node.value
                if target.attr == "bottling_date":
                    self.racking_date = node.value
                if target.attr == "postboil_gravity" and self.original_gravity is None:
                    self.original_gravity = node.value
                if target.attr == "original_gravity" and isinstance(
        ...
        if isinstance(node.func, ast.Attribute) and node.func.attr == "report":
            ...
            if self.original_gravity is not None:
                node.keywords.append(
                    ast.keyword("original_gravity", self.original_gravity)
                )
            if self.racking_date is not None:
                node.keywords.append(ast.keyword("racking_date", self.racking_date))
            if self.final_gravity is not None:
                node.keywords.append(ast.keyword("final_gravity", self.final_gravity))
        if isinstance(node.func, ast.Attribute) and node.func.value.id == "shbf":
    ...
    def visit_Name(self, node):
        if node.id in (
            ...
            return ast.Attribute(
                value=ast.Name("brew", ast.Load()), attr=node.id, ctx=ast.Load()
            )
        return node
