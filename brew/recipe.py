...
class Log(NamedTuple):
    brew_date: str | None
    mash_gravity: float | None
    original_gravity: float | None
    racking_date: str | None
...
class Recipe:
    ...
    def calc_volumes(self, malt_amount: float) -> None:

        if self.batch_size < self.profile.min_batch_size:
            error = f"({self.batch_size:.1f} < {self.profile.min_batch_size:.1f})"
            raise TooSmallBatchError(error)
        boil_volume = (
            self.batch_size
            + self.profile.kettle_deadspace
        ...
        self.mash_volume = boil_volume + self.water_in_malt - self.sparge_volume
    ...
    def calc_gravities(self, report: bool = False) -> None:
        ...
        if report:
            print("Volumes")
        wort = Wort(
            potential_extract * self.profile.mash_efficiency,
            self.mash_volume,
            self.profile,
            report=report,
        )
        ...
        self.late_grav = 1 + wort.extract * late_extract_ratio / wort.volume / 1000
    ...
    def calc_extract(self, report: bool = False) -> tuple[float, float]:
        ...
        def print_extract(label: str, value: float) -> None:
        ...
        if malt_amount > self.profile.max_malt_weight:
            error = f"({malt_amount} kg > {self.profile.max_malt_weight} kg)"
            raise TooMuchMaltError(error)
        self.calc_volumes(malt_amount)
        if report:
            print("  In Mash")
        for f in [f for f in self.fermentables if f.in_mash]:
            potential_extract += f.potential_extract(
                total_amount=malt_amount, report=report
            )
        ...
        if report:
            ...
            effective_extract = potential_extract * self.profile.mash_efficiency
            print_extract(
                f"Effective Extract ({int(self.profile.mash_efficiency * 100)}) %",
                effective_extract,
            )
            print(rfill("   Malt Weight", 34), lfill(f"{malt_amount:.3f} kg", 8))
        ...
        if report and late_extract:
            print("  ", "-" * 54)
            print_extract("Total Potential Extract", potential_extract + late_extract)
            effective_extract = (
                potential_extract * self.profile.mash_efficiency + late_extract
            )
            print_extract("Total Effective Extract", effective_extract)
        return late_extract, potential_extract

    def potential_extract(self) -> float:
    ...
    def utilsation(self, t: int) -> float:
        # Tinseth
        fg = 1.65 * pow(0.000125, self.boil_gravity() - 1)
    ...
    def bitterness(self, report: bool = False) -> float:
        ...
        for hop in self.hops:
            ...
            if report:
                print(
                    rfill(f"   {hop.name} ({hop.alpha} %)", 20),
                    lfill(f"{hop.amount} g", 6),
                    lfill(f"({hop.alpha * hop.amount / 28.3495231:.1f} AAU)", 10),
                    lfill(minstring, 10),
                    lfill(f"{b:.1f}", 5),
                )
            ibu += b
    ...
    def color(self, report: bool = False) -> float:
        ...
        if report:
            print("   --------------------------------------------")
            print(rfill("   MCU", 20), lfill(f"{mcu:.1f}", 25))
            print(rfill("   SRM", 20), lfill(f"{srm:.1f}", 25))
            print(rfill("   EBC", 20), lfill(f"{ebc:.1f}", 25))
        return ebc

    def final_gravity(self) -> float:
        return self.yeast.estimated_fg(self.mash_grav, self.late_grav)

    def abv(self, og: float | None = None, fg: float | None = None) -> float:
        if og is None:
    ...
    def html(self) -> str:
        ...
        for f in self.fermentables:
            if f.in_mash:
                ...
                other += templates.row3(f.name, f"{f.amount:.2f} kg", "in boil")
        ...
        d["stats"] += templates.row2("Target OG", f"{self.original_gravity():.3f}")
        d["stats"] += templates.row2("Target FG", f"{self.final_gravity():.3f}")
        ...
        style = self.style.code if self.style_override is None else self.style_override
    ...
    def heating_time(self, temp_0: int, temp_1: int, volume: float) -> int:
        energy = (temp_1 - temp_0) * volume * 4180
        return int(energy / self.profile.power / 60)

    def time_as_list(self, m: int) -> list[int]:
        h = m // 60
        m %= 60
        return [h, m, 0]

    def time_as_str(self, m: int) -> str:
        h = m // 60
        m %= 60
        s = ""
        if h > 1:
    ...
    def chartdata(self) -> str:
        last_temp = 15
        t = 0
        data = [[[0, 0, 0], last_temp, None, None, None]]
        first = True
        for s in self.mash.steps:
            t += self.heating_time(last_temp, s.temperature, self.mash_volume)
            if first:
                self.heattime = t
                self.mashin = s.temperature
                data.append(
                    [self.time_as_list(t), s.temperature, s.temperature, None, None]
                )
                first = False
            else:
                data.append([self.time_as_list(t), None, s.temperature, None, None])
                t += s.time
                data.append([self.time_as_list(t), None, s.temperature, None, None])
            last_temp = s.temperature

        self.mashtime = t - self.heattime
        data.append([self.time_as_list(t), None, None, s.temperature, None])

        self.boilup = self.heating_time(last_temp, 100, self.boil_volume)
        t += self.boilup
        data.append([self.time_as_list(t), None, None, 100, None])
        t += self.boil_time
        data.append([self.time_as_list(t), None, None, 100, 100])
        t += 30
        data.append([self.time_as_list(t), None, None, None, 20])
    ...
    def program(self) -> str:
        ...
        for s in self.mash.steps:
            if s.name == "Mash In":
                ...
                p += templates.row3(
                    f"Step {i}", f"{s.temperature} °C", f"{s.time:03d} min"
                )
            i += 1
        while i < NSTEPS:
            p += templates.row3(f"Step {i}", f"{s.temperature} °C", "000 min")
            i += 1
        p += templates.row3("Boiling", "100 °C", f"{self.boil_time:03d} min")
        times = {f"{h.minutes: 3d}" for h in self.hops if isinstance(h.minutes, int)}
    ...
    def instructions(self) -> str:
        ...
        for f in self.fermentables:
            if f.in_mash:
                d["malt"] += templates.row2(f.name, f"{f.amount:.2f} kg")
            else:
                d["other"] += templates.row3(
    ...
    def run(self) -> None:
        ...
        parser.add_option(
            "-i", "--instructions", action="store_true", dest="instructions"
        )
        options, args = parser.parse_args(sys.argv)
        path = pathlib.Path(args[0])
        self.design(report=not (options.recipe or options.instructions))
        if options.recipe:
            path.with_stem(path.stem + "_recipe").with_suffix(".html").write_text(
                self.html()
            )
        elif options.instructions:
            path.with_stem(path.stem + "_instructions").with_suffix(".html").write_text(
                self.instructions()
            )
        else:
            self.printlog()
    ...
    def printlog(self) -> None:
        ...
        for (
            ...
            if mash_gravity is not None:
                print(
                    f"  Mash gravity {mash_gravity:.3f}, estimated {self.mash_grav:.3f}"
                )
            ...
            if brew_date is not None and racking_date is not None:
                ferm = datetime.date.fromisoformat(
                    racking_date
                ) - datetime.date.fromisoformat(brew_date)
                print(f"  Fermentation time {ferm.days} days")
            if final_gravity is not None:
                print(f"  FG {final_gravity:.3f}, estimated {self.final_gravity():.3f}")
            if original_gravity is not None and final_gravity is not None:
                print(
                    f"  ABV {self.abv(original_gravity, final_gravity):.1f}"
                    f" %, estimated {self.abv():.1f} %"
                )


class Ingredient:
    def __init__(self, name: str, amount: float, when: int) -> None:
        self.name = name
        self.amount = amount
        self.when = when
...
class SingleStepMash(MashSchedule):
    def __init__(self, temp: int) -> None:
        MashSchedule.__init__(
            self, [Step(60, 0, "Mash In"), Step(temp, 60, "Saccharification rest")]
        )
...
class Step:
    def __init__(self, temperature: int, minutes: int, name: str) -> None:
    ...
    def html(self) -> str:
        if self.time == 0:
            return templates.row3(self.name, "&nbsp;", f"{self.temperature} °C")
        return templates.row3(self.name, f"{self.time} min", f"{self.temperature} °C")
...
class Wort:
    ...
    def add_water(self, volume: float, text: str = "") -> None:
        if self.report:
            print(
                "  ",
                rfill(text, 20),
    ...
    def add_extract(self, extract: float, text: str = "") -> None:
        if self.report:
            print(
                "  ",
                rfill(text, 20),
    ...
    def boil(self, hours: float, text: str = "") -> None:
    ...
    def remove_wort(self, volume: float, text: str = "") -> None:
        if self.report:
            print(
                "  ",
                rfill(text, 20),
    ...
    def println(self, text: str, gravname: str = "") -> None:
        if self.report:
            print("   --------------------------------------------")
            print(
                "  ",
